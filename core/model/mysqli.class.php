...
class mysqli_class extends modelBase implements modelIterface
{
	...
	public function initExec($readOrWrite = "READ")//初始化链接
	{
		...
		if(is_object(Qii::getPrivate('model_mysqli_'. $readOrWrite)))
		{
			return Qii::getPrivate('model_mysqli_'. $readOrWrite);
		}
		/*
		...
		try
		{
			$this->db[$readOrWrite] = $db = mysqli_connect($dbInfo['host'], $dbInfo['user'], $dbInfo['password'], $dbInfo['db']);
		}
		catch (Exception $e) 
		{
			if($readOrWrite == 'READ')
		...
		mysqli_select_db($dbInfo['db']);
		Qii::setPrivate('model_mysqli_'. $readOrWrite, $db);
		return $db;
	}
	...
	public function query($sql)//查询
	{
		...
		{
			mysqli_query("SET CHARACTER SET {$this->sysConfigure['charset']}");
		}
		else 
		{
	...
	public function setError()//设置错误
	{
		if(mysqli_errno($this->db['CURRENT']))
		{
			...
			$this->_errorInfo[$this->_queryTimes]['error'][2] = mysqli_error(mysqli_errno($this->db['CURRENT']));
